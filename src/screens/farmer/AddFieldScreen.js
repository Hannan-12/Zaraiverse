...
export default function AddFieldScreen({ navigation, route }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const fieldData = { name, size, location, userId: user.uid, updatedAt: serverTimestamp() };
      if (fieldToEdit) {
        await updateDoc(doc(db, 'fields', fieldToEdit.id), fieldData);
      } else {
        await addDoc(collection(db, 'fields'), { ...fieldData, createdAt: serverTimestamp() });
      }
      navigation.goBack();
    } catch (error) {
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Save Field</Text>}
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  saveBtn: { backgroundColor: '#2E8B57', padding: 16, borderRadius: 10, alignItems: 'center', marginTop: 10 },
